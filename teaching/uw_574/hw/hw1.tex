...
\title{Ling 575j hw1}
\date{\vspace{-0.2in}Due 11pm on April 6, 2023}
\maketitle
...
A \texttt{Vocabulary} object provides the basic interface between raw text (strings) and integer IDs, which the models we will build require.  In \texttt{/dropbox/22-23/575j/hw1} you will find:
\begin{itemize}
  ...
  \item toy-vocab.txt: output of running 
    ...
    \texttt{python main.py --text\_file /dropbox/22-23/575j/data/toy-reviews.txt \\--output\_file toy-vocab.txt}
\end{itemize}
...
\begin{itemize}
  \item \texttt{python main.py --text\_file /dropbox/22-23/575j/data/sst/train-reviews.txt \\--output\_file train\_vocab\_base.txt}
  \item \texttt{python main.py --text\_file /dropbox/22-23/575j/data/sst/train-reviews.txt \\--output\_file train\_vocab\_freq5.txt --min\_freq 5}
\end{itemize}
